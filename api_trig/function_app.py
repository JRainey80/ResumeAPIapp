...
def main(req: func.HttpRequest) -> func.HttpResponse:
    logging.info('Checking azure-data-tables version.')

    # Retrieve the version of azure-data-tables
    try:
        version = pkg_resources.get_distribution("azure-data-tables").version
        logging.info(f"azure-data-tables version: {version}")
        return func.HttpResponse(f"azure-data-tables version: {version}")
    except pkg_resources.DistributionNotFound:
        return func.HttpResponse("azure-data-tables is not installed", status_code=500)
